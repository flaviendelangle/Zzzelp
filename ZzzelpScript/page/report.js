function ZzzelpScriptRapport() {
	...
	this.etapes = new Array(
		...
		{ nom : 'Récupération de l\'armée', id : 'armee', index : ZzzelpArmy.noms_pluriel },
	...
	this.createInterface = function() {
		...
		for(i=0; i<that.etapes.length; i++) {
			...
			img.src = ZzzelpScript.url + 'Images/suppression.png';
			img.width = '20';
	...
	this.getArmee = function() {
		...
		that.validState(0);	
		if(ComptePlus) {
			ZzzelpArmy.getArmeeReine(that.getNiveaux);
		}
		else {
			ZzzelpArmy.getArmeeAjax(that.getNiveaux);
		}		
	};
	...
	this.validState = function(i) {
		document.querySelectorAll('.zone_imports_zzzelp img')[i].src = ZzzelpScript.url + 'Images/valider.png';
	};
	...
	this.save = function() {
		...
		new ZzzelpAjax({ method : 'POST', domain : 'zzzelp', url : 'rapport_script?alliance=' + galliance + '&', data : form  },
