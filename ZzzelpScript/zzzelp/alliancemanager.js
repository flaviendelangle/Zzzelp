function ZzzelpScriptChaine(zone_page) {
		...
		if(zone.querySelector('#tabMembresAlliance') === null) {
			...
			if (ZzzelpScript.parameters('parametres', ['alliance', 'ally_rangs']) || ZzzelpScript.parameters('parametres', ['alliance', 'ally_couleurs'])) {
				...
				var l = (lieu === 0) ? [2,3] : [1,2],
					...
					rangs_actuels = {},
					tooltips = '';
				for(i=0; i<this.rangs.length; i++) {
					this.rangs[i].alliances = (this.rangs[i].alliances == "*") ? null : this.rangs[i].alliances.replace(/ /g, '').split(',');
					if(this.rangs[i].mode < 2) {
						this.rangs[i].regle = this.rangs[i].regle.replace(/ /g, '').split(',');
					}
				}
				for(i=1; i<lignes.length; i++) {
					...
					that.applyRankUsername(rang, lignes[i], l, pseudo);
					that.applyRankAlliance(rang, lignes[i], l);
					that.applyRankRegExp(rang, lignes[i], l);
					that.applyRankContent(rang, lignes[i], l);
				}
			}
			if(lieu === 0) {
	...
	this.applyRankUsername = function(rang, ligne, l, pseudo) {
		for(n=0; n<this.rangs.length; n++) {
			if(this.rangs[n].alliances === null || ~this.rangs[n].alliances.indexOf(alliance)) {
				if(this.rangs[n].mode === '0' && in_array(pseudo, this.rangs[n].regle)) {
					that.applyRank(ligne, this.rangs[n].rang_affiche, this.rangs[n].couleur, this.rangs[n].role, l);
				}
			}
		}
	};

	this.applyRankAlliance = function(rang, ligne, l) {
		for(n=0; n<this.rangs.length; n++) {
			if(this.rangs[n].alliances === null || ~this.rangs[n].alliances.indexOf(alliance)) {
				if(this.rangs[n].mode == '1' && in_array(alliance, this.rangs[n].regle)) {
					that.applyRank(ligne, this.rangs[n].rang_affiche, this.rangs[n].couleur, this.rangs[n].role, l);
				}
			}
		}		
	...
	this.applyRankRegExp = function(rang, ligne, l) {
		for(n=0; n<this.rangs.length; n++) {
			if(this.rangs[n].alliances === null || ~this.rangs[n].alliances.indexOf(alliance)) {
				if(this.rangs[n].mode == '2' && rang.match(this.rangs[n].regle)) {
					var rep = new RegExp(this.rangs[n].regle).exec(rang);
					var resultat = this.rangs[n].rang_affiche;
					for(t=1;t<rep.length; t++) {
						resultat = resultat.replace('$' + (t), rep[t]);
					}
					that.applyRank(ligne, resultat, this.rangs[n].couleur, this.rangs[n].role, l);					
				}
			}
		}		
	};

	this.applyRankContent = function(rang, ligne, l) {
		for(n=0; n<this.rangs.length; n++) {
			if(this.rangs[n].alliances === null || ~this.rangs[n].alliances.indexOf(alliance)) {
				if(this.rangs[n].mode == '3' && ~rang.indexOf(this.rangs[n].regle)) {
					that.applyRank(ligne, this.rangs[n].rang_affiche, this.rangs[n].couleur, this.rangs[n].role, l);
				}
			}
		}		
